...
__global__ void blockSymmetricSpMVCOOBackward(const double* diagonalBlockValues, const double* offDiagonalBlockValues, const int* blockOuterIndices, const int* blockInnerIndices, const double* x, double* y, double* yt, int nRows, int outerSize){
    int id = blockIdx.x * blockDim.x + threadIdx.x;
    int tid = threadIdx.x;
    __shared__ double allBlocks[BLOCKSIZE * BLOCKSIZE * 32];
    __shared__ double allResults[BLOCKSIZE * 32];
    ...
    if (id < outerSize){
        for (int i = 0; i < BLOCKSIZE * BLOCKSIZE; i++){
            allBlocks[tid * BLOCKSIZE * BLOCKSIZE + i] = *(offDiagonalBlockValues + (outerSize - 1 - id) * BLOCKSIZE * BLOCKSIZE + i);
        }
        for (int i = 0; i < BLOCKSIZE; i++){
            allResults[tid * BLOCKSIZE + i] = 0.0;
        }

        rows[tid] = blockOuterIndices[(outerSize - 1 - id)];
        cols[tid] = blockInnerIndices[(outerSize - 1 - id)];
    }
    __syncthreads();
    if (id < outerSize){
        int col =cols[tid];
...
__global__ void blockSymmetricSpMVCOOFCFSBackward(const double* diagonalBlockValues, const double* offDiagonalBlockValues, const int* blockOuterIndices, const int* blockInnerIndices, const double* x, double* y, double* yt, int nRows, int outerSize){
    int tid = threadIdx.x;
    __shared__ double allBlocks[BLOCKSIZE * BLOCKSIZE * 32];
    __shared__ double allResults[BLOCKSIZE * 32];
    __shared__ int rows[32];
    ...
    if (queueID < outerSize){
        for (int i = 0; i < BLOCKSIZE * BLOCKSIZE; i++){
            allBlocks[tid * BLOCKSIZE * BLOCKSIZE + i] = *(offDiagonalBlockValues + (outerSize - 1 - queueID) * BLOCKSIZE * BLOCKSIZE + i);
        }
        for (int i = 0; i < BLOCKSIZE; i++){
            allResults[tid * BLOCKSIZE + i] = 0.0;
        }

        rows[tid] = blockOuterIndices[(outerSize - 1 - queueID)];
        cols[tid] = blockInnerIndices[(outerSize - 1 - queueID)];
    }
    __syncthreads();
    if (queueID < outerSize){
        int col =cols[tid];
...
int main() {
    ...
    // use coo method but we do the work in the backward order
    // set y to zero
    setZero<<<(nRows + 32) / 32, 32>>>(d_y, nRows);
    cudaDeviceSynchronize();
    cudaEventRecord(start);
    for (int i = 0; i < NUM_EXECUTION; i++){
        ...
        setZero<<<(nRows + 32) / 32, 32>>>(d_y, nRows);
        blockSymmetricSpMVCOOBackward<<<(blockFullOuterIndices.size() + 32) / 32, 32>>>(d_diagonalBlockValues, d_offDiagonalBlockValues, d_blockFullOuterIndices, d_blockInnerIndices, d_x, d_y, d_yt, nRows, blockFullOuterIndices.size());
        addDaigonal<<<(nRows + 32) / 32, 32>>>(d_diagonalBlockValues, d_x, d_y, d_yt, nRows);
        cudaDeviceSynchronize();
    }
    cudaEventRecord(stop);
    ...
    cudaEventElapsedTime(&milliseconds, start, stop);
    printf("Block SpMV COO Backward time for %d executions: %f ms\n", NUM_EXECUTION, milliseconds);
    // Copy result back to host
    cudaMemcpy(computedResult.data(), d_y, computedResult.size() * sizeof(double), cudaMemcpyDeviceToHost);
    ...
    // use coo method but but with first come first serve
    // set y to zero
    setZero<<<(nRows + 32) / 32, 32>>>(d_y, nRows);
    cudaDeviceSynchronize();
    cudaEventRecord(start);
    ...
    cudaEventElapsedTime(&milliseconds, start, stop);
    printf("Block SpMV COO FCFS time for %d executions: %f ms\n", NUM_EXECUTION, milliseconds);
    // Copy result back to host
    cudaMemcpy(computedResult.data(), d_y, computedResult.size() * sizeof(double), cudaMemcpyDeviceToHost);
    ...
    // use coo method but but with first come first serve also backward
    // set y to zero
    setZero<<<(nRows + 32) / 32, 32>>>(d_y, nRows);
    cudaDeviceSynchronize();
    cudaEventRecord(start);
    for (int i = 0; i < NUM_EXECUTION; i++){
        ...
        cudaMemcpyToSymbol(globalID, &zero, sizeof(int));
        setZero<<<(nRows + 32) / 32, 32>>>(d_y, nRows);
        blockSymmetricSpMVCOOFCFSBackward<<<(blockFullOuterIndices.size() + 32) / 32, 32>>>(d_diagonalBlockValues, d_offDiagonalBlockValues, d_blockFullOuterIndices, d_blockInnerIndices, d_x, d_y, d_yt, nRows, blockFullOuterIndices.size());
        addDaigonal<<<(nRows + 32) / 32, 32>>>(d_diagonalBlockValues, d_x, d_y, d_yt, nRows);
        cudaDeviceSynchronize();
    }
    cudaEventRecord(stop);
    ...
    cudaEventElapsedTime(&milliseconds, start, stop);
    printf("Block SpMV COO FCFS backward time for %d executions: %f ms\n", NUM_EXECUTION, milliseconds);
    // Copy result back to host
    cudaMemcpy(computedResult.data(), d_y, computedResult.size() * sizeof(double), cudaMemcpyDeviceToHost);
